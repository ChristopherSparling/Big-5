...
train_x <- data.matrix(big5$x[train_indices,])
train_y <- data.matrix(big5$y[train_indices,])
str(train_x)
str(train_y)
...
# Model creation
inputs <- layer_input(shape = c(5))
model <- keras_model_sequential()

model %>%
  layer_dense(units = 7, input_shape = 5, name = "Input_Layer") %>%
  layer_activation(activation = 'relu') %>%
  layer_dense(units = 5, name = "Dense_2") %>%
  layer_activation(activation = 'relu') %>%
  layer_dense(units = 7, name = "Dense_3") %>%
