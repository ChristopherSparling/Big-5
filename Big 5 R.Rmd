...
model %>%
  layer_dense(units = 5, 
              input_shape = c(5), 
              name = "Input_Layer") %>% #,kernel_regularizer = regularizer_l1()
  layer_activation(activation = 'sigmoid') %>%
  layer_dense(units = 7,
              name = "Dense_4") %>% #,kernel_regularizer = regularizer_l1()
  layer_activation(activation = 'sigmoid')

model %>% compile(
  loss = loss_binary_crossentropy,
  optimizer = optimizer_rmsprop(lr = 0.1, decay = 0.001), # Modified learning rate, being checked in logarithmic steps
